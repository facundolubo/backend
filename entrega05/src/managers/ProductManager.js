...
class ProductManager {
    constructor() {
        this.path = '/home/facundol/Documents/Computacion/coderhouse/backend/entrega05/data/products.json'
    }
    ...
    /* Method that gets the content of the products file returns the following.
        ...
        This method read the whole file.*/
    getContentFile = async () => {
        console.log('ProductManager.getContentFile')
        let content = ""
        try {
    ...
    setContentFile = async content => {
        console.log('ProductManager.setContentFile')
        try {
    ...
    getProducts = async nProducts => {
        ...
        // va a leer todo el archivo porque no hay un metodo para leer solo los campos correspondientes
        const content = await this.getContentFile()

        if (content === false) {
            return false
        }
        // creamos la lista con todo el archivo
        const products = new ListProducts()
        products.setList(content)
        // retornamos solo los productos de la lista solicitados
    ...
    updateProductById = async (idProduct, dataUpdate) => {
        console.log('ProductManager.updateProductById')
        const content = await this.getContentFile()
    ...
    deleteProductById = async idProduct => {
        console.log('ProductManager.deleteProductById')
        const content = await this.getContentFile()
